  ...
  return (
    <TooltipProvider delay={300}>
      <main className="flex h-dvh min-h-[560px] flex-col overflow-hidden bg-background text-foreground">
        <header className="grid min-h-16 grid-cols-[minmax(220px,1fr)_auto_minmax(220px,1fr)] items-center border-b border-border bg-background px-5 max-md:grid-cols-[minmax(0,1fr)_auto_auto] max-sm:min-h-[58px] max-sm:px-3">
          <div className="flex min-w-0 items-baseline gap-3 max-sm:hidden"><h1 className="truncate font-heading text-[22px] font-medium tracking-[-0.025em]">JSTOR AI Ethics Atlas</h1></div>
          <nav className="flex self-stretch" aria-label="Atlas views">
            <button onClick={() => setWorkspace('papers')} className={`border-b-2 px-4 text-xs font-medium transition-colors ${workspace === 'papers' ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>Papers</button>
            <button onClick={() => setWorkspace('correlations')} className={`border-b-2 px-4 text-xs font-medium transition-colors ${workspace === 'correlations' ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>Correlation matrices</button>
          </nav>
