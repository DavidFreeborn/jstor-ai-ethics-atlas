...
import { AGREEMENT_STOPS, CATEGORY_COLOURS } from '@/lib/atlas-visual';
...
export function PaperMap({ data, lens, selected, onSelect, topicFilter, selections, colourSlots, relation }: {
  ...
  const markFor = useCallback((paper: PaperPoint): Mark => {
    ...
    if (lens === 'publisher') { const applicable = Boolean(paper.publisher) && selectedValues.publisher.has(paper.publisher); return { colours: applicable ? [CATEGORY_COLOURS[colourSlots.publisher.get(paper.publisher) ?? 0]] : [], applicable }; }
    if (lens === 'journal') { const applicable = Boolean(paper.journal) && selectedValues.journal.has(paper.journal); return { colours: applicable ? [CATEGORY_COLOURS[colourSlots.journal.get(paper.journal) ?? 0]] : [], applicable }; }
    if (lens === 'keywords') { const matches = paper.keywords.filter((value) => selectedValues.keywords.has(value)); return { colours: matches.map((value) => CATEGORY_COLOURS[colourSlots.keywords.get(value) ?? 0]), applicable: matches.length > 0 }; }
  ...
  useEffect(() => {
    ...
    if (selectedScreen) { for (const id of relation.drawn) { const target = pointById.get(id); if (!target) continue; const sharedCount = relation.shared.get(id)?.length ?? 1; context.beginPath(); context.moveTo(selectedScreen.x, selectedScreen.y); context.lineTo(target.x, target.y); context.strokeStyle = `rgba(235,244,250,${Math.min(0.35, 0.10 + sharedCount * 0.05)})`; context.lineWidth = Math.min(1.6, 0.55 + sharedCount * 0.18); context.stroke(); } }
    const radius = Math.min(3.8, 1.8 + Math.log2(Math.max(1, transform.scale)) * 0.55);
    for (const point of screenPoints) { if (point.x < -8 || point.y < -8 || point.x > size.width + 8 || point.y > size.height + 8) continue; const mark = markFor(point.paper); const connected = !hasRelation || point.paper.id === selected?.id || relation.all.has(point.paper.id); const alpha = mark.applicable && connected ? 0.96 : 0.16; drawMark(context, point.x, point.y, mark.colours.length > 1 ? radius + 0.7 : radius, mark.colours, alpha); if (hasRelation && relation.all.has(point.paper.id)) { context.beginPath(); context.arc(point.x, point.y, radius + 1.4, 0, Math.PI * 2); context.strokeStyle = 'rgba(255,255,255,.55)'; context.lineWidth = 0.65; context.stroke(); } }
