...
import { BookOpenText, Search, SlidersHorizontal, X } from 'lucide-react';
...
const LENSES: { id: PaperLens; label: string; description: string }[] = [
  { id: 'bertopic', label: 'Semantic topics', description: '26-topic BERTopic solution' },
  { id: 'bertopic_reduced', label: 'Broad topics', description: 'Nine-topic reduction' },
  { id: 'provenance', label: 'Assignment', description: 'Core, reassigned and outlier' },
  { id: 'lda', label: 'LDA topics', description: 'Dominant mixed-membership topic' },
  { id: 'lda_dominance', label: 'LDA focus', description: 'Strength of the dominant topic' },
  { id: 'alignment', label: 'Cross-method', description: 'Local BERTopic–LDA alignment' },
];
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border px-4 py-3">
        <p className="data-kicker">Paper lens</p>
        <div className="mt-2 space-y-0.5">
          {LENSES.map((item) => (
            <button key={item.id} className={`group w-full border-l-2 px-2 py-1.5 text-left transition-colors ${lens === item.id ? 'border-primary bg-background text-foreground' : 'border-transparent text-muted-foreground hover:bg-background/60 hover:text-foreground'}`} onClick={() => onLens(item.id)}>
              <span className="block text-[13px] font-medium">{item.label}</span>
              <span className="block truncate text-[10px] leading-4 opacity-75">{item.description}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-auto border-t border-border px-4 py-3 text-[10px] leading-relaxed text-muted-foreground"><p><span className="font-mono text-foreground">{data.cohort.n.toLocaleString()}</span> model-ready abstracts</p><p className="mt-1">Positions stay fixed across lenses.</p></div>
    </div>
  );
}
...
export function PapersWorkspace({ data }: { data: MapData | null }) {
  ...
  const [selected, setSelected] = useState<PaperPoint | null>(null);
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)] max-md:grid-cols-[176px_minmax(0,1fr)] max-sm:grid-cols-1">
      ...
      <section className="relative min-h-0 overflow-hidden" aria-label="Paper atlas">
        <PaperMap data={data} lens={lens} selected={selected} onSelect={setSelected} topicFilter={topicFilter} />
        <div className="absolute left-3 top-3 flex gap-2">
          <Sheet><SheetTrigger render={<Button size="sm" variant="outline" className="hidden rounded-none bg-background/92 max-sm:inline-flex" />}><SlidersHorizontal />Lens</SheetTrigger><SheetContent side="left" className="w-[290px]"><SheetHeader><SheetTitle>Paper lenses</SheetTitle><SheetDescription>One fixed semantic geometry, recoloured by analytical result.</SheetDescription></SheetHeader><LensControls data={data} lens={lens} topicFilter={topicFilter} onLens={changeLens} onTopic={setTopicFilter} /></SheetContent></Sheet>
          <div className="pointer-events-none border border-border/75 bg-background/88 px-3 py-2 backdrop-blur-sm max-sm:hidden"><p className="data-kicker">{LENSES.find((item) => item.id === lens)?.label}</p><p className="mt-1 max-w-[260px] text-[11px] text-muted-foreground">{data.geometry.embedding} · fixed two-dimensional projection</p></div>
        </div>
        <div className="absolute right-3 top-3 z-20 w-[min(330px,calc(100%-88px))]">
          ...
          {query ? <div className="mt-1 border border-border bg-background shadow-lg">{results.length ? <ul className="py-1" aria-label="Search results">{results.map((paper) => <li key={paper.id}><button className="w-full border-l-2 border-transparent px-3 py-2 text-left hover:border-primary hover:bg-muted/60 focus-visible:border-primary focus-visible:bg-muted/60 focus-visible:outline-none" onClick={() => { setSelected(paper); setQuery(''); }}><span className="block truncate text-sm font-medium">{paper.title}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{paper.journal || paper.id}</span></button></li>)}</ul> : <p className="px-3 py-4 text-sm text-muted-foreground">No matching papers</p>}</div> : null}
        </div>
        {selected ? <aside className="absolute bottom-4 right-4 top-14 z-20 flex w-[min(360px,calc(100%-32px))] flex-col border border-border bg-background/97 shadow-xl backdrop-blur-sm"><div className="flex items-start justify-between border-b border-border px-4 py-3"><div className="min-w-0 pr-3"><p className="data-kicker">Selected paper</p><h2 className="mt-1.5 font-heading text-xl font-medium leading-tight">{selected.title}</h2></div><Button variant="ghost" size="icon-sm" aria-label="Close paper details" onClick={() => setSelected(null)}><X /></Button></div><ScrollArea className="min-h-0 flex-1"><div className="space-y-5 p-4"><div><p className="text-xs text-muted-foreground">{selected.journal || 'Journal unavailable'}</p><a href={selected.id} target="_blank" rel="noreferrer" className="mt-1 block truncate font-mono text-[10px] text-primary underline decoration-border underline-offset-2 hover:decoration-primary">{selected.id}</a></div><div className="border-y border-border py-3"><p className="data-kicker">Active lens</p><p className="mt-1 text-sm font-medium">{lensValue(selected, lens, topics)}</p></div>{selected.preview ? <p className="text-[13px] leading-6 text-foreground/82">{selected.preview}…</p> : null}<dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-xs"><div><dt className="text-muted-foreground">BERTopic</dt><dd className="mt-0.5 font-medium">{data.topics.bertopic.find((topic) => topic.id === selected.bertopic)?.label}</dd></div><div><dt className="text-muted-foreground">Assignment</dt><dd className="mt-0.5 font-medium capitalize">{selected.provenance}</dd></div><div><dt className="text-muted-foreground">LDA</dt><dd className="mt-0.5 font-medium">{selected.lda ? data.topics.lda.find((topic) => topic.id === selected.lda?.topic)?.label : 'Unavailable'}</dd></div><div><dt className="text-muted-foreground">LDA dominance</dt><dd className="mt-0.5 font-mono font-medium">{selected.lda ? `${Math.round(selected.lda.dominance * 100)}%` : '—'}</dd></div></dl>{selected.lda ? <section className="border-t border-border pt-4"><div className="flex items-center justify-between"><p className="data-kicker">LDA composition</p><span className="font-mono text-[9px] text-muted-foreground">memberships ≥10%</span></div><ol className="mt-3 space-y-2">{selected.lda.memberships.map((membership) => { const topic = data.topics.lda.find((item) => item.id === membership.topic); return <li key={membership.topic}><div className="flex items-end justify-between gap-3 text-[10px]"><span className="truncate">{topic?.label ?? `Topic ${membership.topic}`}</span><span className="font-mono">{Math.round(membership.value * 100)}%</span></div><div className="mt-1 h-1 bg-muted"><div className="h-full" style={{ width: `${membership.value * 100}%`, backgroundColor: topic?.colour }} /></div></li>; })}</ol></section> : null}</div></ScrollArea></aside> : null}
        <div className="pointer-events-none absolute bottom-3 right-4 flex items-center gap-2 text-[10px] text-muted-foreground"><BookOpenText className="size-3" /><span>{data.cohort.n.toLocaleString()} mapped · {data.cohort.catalogue_n.toLocaleString()} catalogue</span></div>
      </section>
    </div>
  );
}
